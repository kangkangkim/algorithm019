...
public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet) {
        ...
        //month -2 because the index 2 is January
        ...
        //get the possibilites of drywet and wetwet
        double p_drywet=0.0;
        double p_wetwet=0.0;
        //size of drywet = size of wetwet
        for(int i=0;i<drywet.length;i++){
            for(int j=0;j<drywet[i].length;j++){
                
                if(drywet[i][0]==longitute && drywet[i][1]==latitude){
                    //for Feb index is 3
                    p_drywet =drywet[i][month];
                }
                if(wetwet[i][0]==longitute && wetwet[i][1]==latitude){

                    p_wetwet=wetwet[i][month];
                }
            }
        }

        // for first day
        double ran=StdRandom.uniform();// get a value [0,1)
        //0-0.49 (wet), 0.50-0.99 (dry)
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        for(int i=0;i<forecast.length;i++){
            ...
            //if i is the last number
            //this step is always ignored
